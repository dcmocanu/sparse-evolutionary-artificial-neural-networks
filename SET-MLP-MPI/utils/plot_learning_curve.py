...
set_mlp=np.loadtxt("../Results/set_mlp_mpi_fashionmnist_60000_training_samples_e20_rand1_process_0_num_workers_6.txt")
...
ax1.plot([i for i in range(1, 201, 1)],set_mlp[:,2]*100, label="SET-MLP MPI keras one cpu train accuracy", color="r")
ax1.plot([i for i in range(1, 201, 1)],set_mlp[:,3]*100, label="SET-MLP MPI keras one cpu test accuracy", color="b")
...
ax2.plot([i for i in range(1, 201, 1)],set_mlp[:,0], label="SET-MLP MPI keras one cpu train loss", color="r")
ax2.plot([i for i in range(1, 201, 1)],set_mlp[:,1], label="SET-MLP MPI keras one cpu test loss", color="b")
...
plt.savefig("../Plots/SET_keras_one_cpu_60000_fashionmist.pdf", bbox_inches='tight')
