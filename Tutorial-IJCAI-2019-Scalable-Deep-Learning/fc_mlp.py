...
class FC_MLP:
    def __init__(self, dimensions, activations):
        ...
        for i in range(len(dimensions) - 1):
            self.w[i + 1] = np.random.randn( dimensions[i],dimensions[i + 1])/ 10
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        fc_mlp = FC_MLP((X_train.shape[1], noHiddenNeuronsLayer, noHiddenNeuronsLayer,noHiddenNeuronsLayer, Y_train.shape[1]), (Relu, Relu,Relu,  Sigmoid))

        # train FC-MLP
        fc_mlp.fit(X_train, Y_train, X_test, Y_test, loss=MSE, epochs=noTrainingEpochs, batch_size=batchSize, learning_rate=learningRate,
                    momentum=momentum, weight_decay=weightDecay,  dropoutrate=dropoutRate, testing=True,
                    save_filename="Results/fc_mlp_"+str(noTrainingSamples)+"_training_samples_rand"+str(i)+".txt")
